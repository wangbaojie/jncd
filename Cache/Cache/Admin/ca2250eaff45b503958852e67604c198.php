...
<div id="header" class="header">
	<div class="logo"><img src="./Public/Images/admin_logo.gif" width="180" ></div>
	<div class="nav f_r"> <a href="javascript:void(0);" onclick="gocacheurl();"><?php echo L('UPDATE_CACHE');?></a> &nbsp;&nbsp;&nbsp;&nbsp;</div>
    	...
    	<div id="leftMenu">
			<div style="padding-left:12px;_padding-left:10px;">				
				<?php if(is_array($menu)): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><dl id="nav_<?php echo ($r['bnav']['id']); ?>">
						...
						<?php if(is_array($r[nav])): $i = 0; $__LIST__ = $r[nav];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><dd id="nav_<?php echo ($item['id']); ?>"><span  onclick="javascript:gourl('<?php echo ($item['id']); ?>','<?php echo U($item['model'].'/'.$item['action'],$item['data']);?>')"><a href="<?php echo U($item['model'].'/'.$item['action'],$item['data']);?>" target="main"><?php echo ($item['name']); ?></a><?php if($r[bnav]['model']=='Category') : ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <?php if($item['id']=='77') : ?>
                        <?php else :?>
                         <a href="<?php echo U($item['model'].'/add',$item['data']);?>" id="<?php echo ($i); ?>" target="main"> <?php echo L(add);?></a>
                        <?php endif;?>
                       <?php endif;?></span></dd><?php endforeach; endif; else: echo "" ;endif; ?>
