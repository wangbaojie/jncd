...
<div class="main">
    ...
    <?php  $arrparentid = array_filter(explode(',', $Categorys[$catid]['arrparentid'].','.$catid));foreach($arrparentid as $cid):$parsestr[] = '<a href="'.$Categorys[$cid]['url'].'">'.$Categorys[$cid]['catname'].'</a>'; endforeach;echo implode(" &gt; ",$parsestr);?></h1>
        	...
        	<div class="main-pro">
        	<ul>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><li style="height: auto;width:180px;"><a <?php if($catid!=7) : ?>href="<?php echo ($r["url"]); ?>"<?php endif;?>><img  style="width: 180px;height: 250px;"  src="<?php echo ($r["thumb"]); ?>"><p style="width:180px;"><?php echo (str_cut($r["title"],30)); ?></p></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
            </ul>
            <div class="clear"></div>
