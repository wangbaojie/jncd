...
<div class="main">
    ...
    <?php  $arrparentid = array_filter(explode(',', $Categorys[$catid]['arrparentid'].','.$catid));foreach($arrparentid as $cid):$parsestr[] = '<a href="'.$Categorys[$cid]['url'].'">'.$Categorys[$cid]['catname'].'</a>'; endforeach;echo implode(" &gt; ",$parsestr);?></h1>
        ...
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><div class="ybox">
          <div class="ybox_left"><a href="<?php echo ($r["url"]); ?>" title=""><img src="<?php echo ($r["thumb"]); ?>" alt=""></a></div>
          <div class="ybox_right">
            <h3><a href="<?php echo ($r["url"]); ?>" title=""><?php echo (str_cut($r["title"],70)); ?></a></h3>
            <p>&nbsp; &nbsp; &nbsp;&nbsp;<?php echo (str_cut(strip_tags($r["description"]),150)); ?> </p>
            <div class="ybox_more"><span><i class="icon_time"></i><?php echo (todate($r["createtime"],'Y-m-d')); ?></span><span><i class="icon_view"></i><a href="<?php echo ($r["url"]); ?>" title="">查看详情</a></span></div>
          </div>
        </div><?php endforeach; endif; else: echo "" ;endif; ?>
      </div>
          <div id="pages" class="page"><?php echo ($pages); ?></div>
         <style type="text/css">
		 .rightCon,.RNewCon{ float:right; width:840px;}
		 .RNewCon a{ color:#000;}
		 .ybox{ overflow:hidden;height:130px; padding:20px 0; border-bottom:1px dotted #ccc;}
.ybox_left{ float:left; width:200px;}
.ybox_left img{ display:block; width:200px; height:120px;}
.ybox_right{ float:right; width:620px;}
.ybox_right h3{ height:32px; line-height:32px;}
.ybox_right h3 a{ font-size:18px; font-weight:600}
.ybox_right p{ height:60px; line-height:140%;}
.ybox_more{ text-align:right; width:100%;}
.ybox_more span{ margin-left:25px;}
